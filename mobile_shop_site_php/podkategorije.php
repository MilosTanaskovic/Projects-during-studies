...
<div id="mySidenav" class="sidenav">
    ...
    <button class="btn btn-primary btn-block" name="btnLogIn">Prijava </button>
	  ...
	  <div id="sadrzaj_centar">
	    <?php 
		 @$id_proizvodjac=$_GET['id'];
		 if(isset($id_proizvodjac)){
		 $upit2="SELECT m.id_model,m.naziv_modela,COUNT(m.id_model) as broj 
		 FROM model m, telefon t
		 WHERE m.id_model=t.id_model
		 AND m.id_proizvodjac=".$id_proizvodjac."
		 GROUP BY m.naziv_modela, m.id_model";
		 
		 $rezulta2=mysql_query($upit2,$konekcija) or die("Greska-upit2".mysql_error());
		 
		 $broj=mysql_num_rows($rezulta2);
		 
		 if($broj==0){
		  echo "Trenutno nema modela sa ovim proizvodjacom";
		 }
		 else{
		 
		 echo "<ul class='krug'>";
		  while($red2=mysql_fetch_array($rezulta2)){
		  
		  echo "<li style='margin:10px'><a href='prikaz.php?id=".$red2['id_model']."'class='btn btn-warning'>".$red2['naziv_modela']."</a>&nbsp;<kbd>".$red2['broj']."</kbd></li>";
		 }
		 echo "</ul>";
		 }
		 }
		?>
		<img src="slike/jabuka.jpg" alt="apple" class="img-thumbnail" style="margin-top:70px" width="500px">
	  ...
	  <div id="sadrzaj_desno">
	   ...
	   <div id="sadrzaj_desno_div">
		...
		while($red4=mysql_fetch_array($rezulatat4)){
			 ...
			 echo"<tr><td><a href='prikaz.php?id=".$red4['id_model']."'class='btn btn-info'>".$red4[1]."-".$red4['naziv_modela']."</a></br><samp>Cena: ".$red4['min_cena'].".00&nbspdin.</samp></td></tr>";
		}
		echo "</table>";
		include('kraj.php');
		?>
